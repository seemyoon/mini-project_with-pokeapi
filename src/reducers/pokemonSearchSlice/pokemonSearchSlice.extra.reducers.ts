...
export const loadPokemonByTypes = createAsyncThunk(
    "pokemonSearchByTypes",
    async (nameType: string, thunkAPI) => {
        try {
            const response = await pokemonSearchService.getPokemonByTypes(nameType)
            return thunkAPI.fulfillWithValue(response)
        } catch (error) {
            const axiosError = error as AxiosError
            if (axiosError) {
